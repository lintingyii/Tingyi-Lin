let colors = "0e131f-38405f-59546c-8b939c-0000ff".split("-").map(a=>"#"+a)
// let colors = "fff-fff-f13".split("-").map(a=>"#"+a)
// let colors = "fff-1be7ff-6eeb83-e4ff1a-ffb800-ff5714".split("-").map(a=>"#"+a)
let overAllTexture
class Wave{
	constructor(args){
		let def ={
			p: createVector(0,0),
			rSpeed: random(2)*random(),
			color: color(0),
			r: 0,
			live: int(random(100,300)),
			alive: true,
			polygonCount: int(random(1,35)),
			randomId: int(random(5000)),
			dash: random()<0.6,
	...
	draw(){
		...
		beginShape()
			for(let i=0;i<100;i++){
				let rr = map(noise(i,frameCount/50,this.randomId),0,1,0.9,1.1)*this.r
				let ang = i+ random()*0.
